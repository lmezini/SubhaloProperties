...
rad_cut = np.logspace(np.log10(0.001), np.log10(1.), 100)#[0.025,0.05,0.075,0.1]
...
for j,f in enumerate(halo_names):
    ...
    if len(particlevalues) > 0:
        ...
        for k in range(len(rad_cut)):
            ...
            mass_frac_A[j][k]+=particle_mass*len(pos[rad_dist<cut_off])/mvirs[j]

        for n in range(100):
            rand_rad_dist, angle = rotate_position(host_I[j],pos,rvirs[j])
            for k in range(len(rad_cut)):
                cut_off = 0.001*rvirs[j]*rad_cut[k]
                mass_frac_random[m][k]+=particle_mass*len(pos[rand_rad_dist<cut_off])/mvirs[j]
    ...
    np.savez('rhap_mass_frac_rad_cut_mass_cut_1_all_rad.npz', A = mass_frac_A, rand = mass_frac_random, angle = random_angle)
