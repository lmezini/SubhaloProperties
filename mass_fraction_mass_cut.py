...
mass_frac_A = np.zeros((len(host_vals),len(ang_cut)))
mass_frac_random = np.zeros((int(len(host_vals)*100),len(ang_cut)))
...
for j,f in enumerate(halo_names):
    ...
    mass_cut = hostvalues['mvir']> 0.02*mvirs[j]
    hostvalues = hostvalues[mass_cut]

    particlevalues = pd.DataFrame()

    for h in hostvalues:
        p_set = particles["smallest_external_haloid"] == h['id']

        if len(particles[p_set])>0:
            particle_set = particles[p_set].to_pandas()
            particlevalues = pd.concat([particlevalues,particle_set],ignore_index=True)

    particlevalues = Table.from_pandas(particlevalues)
    ...
    if len(particlevalues) > 0:
        ...
        t = np.arccos(abs((pos*hA2).sum(axis=1)/(norm(pos,axis=1)*norm(hA))))*180./np.pi

        for k in range(len(ang_cut)):
            mass_frac_A[j][k]+=particle_mass*len(pos[t<ang_cut[k]])/mvirs[j]

        for n in range(100):
            rand_t = rotate_position(host_I[j],pos,rvirs[j])*180/np.pi
            for k in range(len(ang_cut)):
                mass_frac_random[m][k]+=particle_mass*len(pos[rand_t<ang_cut[k]])/mvirs[j]

            m+=1 #this is in n loop
    ...
    np.savez('rhap_mass_frac_mass_cut_02.npz', A = mass_frac_A, rand = mass_frac_random)
